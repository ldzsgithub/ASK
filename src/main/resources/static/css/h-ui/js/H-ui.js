...
(function($){
	...
	var config = $.cookie = function(key, value, options) {
		...
		if (arguments.length > 1 && !$.isFunction(value)) {
			...
			return (document.cookie = [encode(key), '=', stringifyCookieValue(value), options.expires ? '; expires=' + options.expires.toUTCString() : '', // user expires attribute, max-age is not supported by IE
...
(function(factory) {
	...
	/*
			Usage Note:
			-----------
			Do not user both ajaxSubmit and ajaxForm on the same form.  These
			...
			You can also user ajaxForm with delegation (requires jQuery v1.7+), so the
	...
	$.fn.ajaxSubmit = function(options) {
		...
		// convenient for user with rich editors like tinyMCE or FCKEditor
		var veto = {};
		...
		function deepSerialize(extraData) {
			...
			for (i = 0; i < len; i++) {
				...
				// #278; user array instead of object storage, favoring array serializations
		...
		function fileUploadIframe(a) {
			...
			var toXml = $.parseXML ||
			function(s, doc) { // user parseXML if available (jQuery 1.5+)
	...
	$.fn.formToArray = function(semantic, elements) {
		...
		// #386; account for inputs outside the form which user the 'form' attribute
		if (formId) {
...
! (function($, window, document, undefined) {
	...
	$.fn.lazyload = function(options) {
		...
		this.each(function() {
			...
			/* If no src attribute given user data:uri. */
	...
	*/
		...
		*/
			this.options = {
				...
				openPos: "relative",
				// String: Position of the opened nav, relative or css
			...
			if (document.getElementById(this.wrapperEl)) {
				...
				// If element with an ID doesn't exist, user querySelector
		...
		*/
			_createToggle: function() {
				...
				if (!opts.customToggle) {
					...
					// There is a toggle already, let's user that one
				} else {
...
function($) {
	...
	Popover.prototype.setContent = function() {
		...
		$tip.find('.popover-content').children().detach().end()[ // we user append for html objects to maintain js events
		...
		function createNewSlider(element, options) {
			...
			**************************************************/
			...
			for (var i = 0; i < optionTypes.length; i++) {
				...
				// Finally, if nothing was specified, user the defaults
		...
		Slider.prototype = {
			...
			********************************/
			...
			_keydown: function(handleIdx, ev) {
				...
				// user natural arrow keys instead of from min to max
