...
/* 
 * See www.highcharts.com/studies/world-map.htm for user case.
 *
 * To do:
...
(function (Highcharts) {
	...
	/**
	 * Override to user the extreme coordinates from the SVG shape, not the
	 * data values
	 */
	wrap(Axis.prototype, 'getSeriesExtremes', function (proceed) {
		...
		// Remove the xData array and cache it locally so that the proceed method doesn't user it
	...
	seriesTypes.map = Highcharts.extendClass(seriesTypes.scatter, {
		...
		/**
		 * If neither valueRanges nor colorRanges are defined, user basic area symbol.
		 */
		drawLegendSymbol: seriesTypes.area.prototype.drawLegendSymbol,
		...
		getBox: function (paths) {
			...
			each(paths || this.options.data, function (point) {
				...
				// Cache point bounding box for user to position data labels
		...
		translateColors: function (dataMin, dataMax) {
			...
			each(this.data, function (point) {
				...
				if (color) {
					point.color = null; // reset from previous drilldowns, user of the same data options
