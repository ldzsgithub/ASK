...
/**
 * Set or get an attribute or an object of attributes. Can't user jQuery attr because
...
function getTimeTicks(normalizedInterval, min, max, startOfWeek) {
	...
	if (defined(min)) { // #1300
		...
		while (time < max) {
			...
			// if the interval is years, user Date.UTC to increase years
			if (interval === timeUnits[YEAR]) {
				time = makeTime(minYear + i * count, 0);
	
			// if the interval is months, user Date.UTC to increase months
			...
			// else, the interval is fixed and we user simple addition
			} else {
				time += interval * count;
			}
	...
	win.HighchartsAdapter = win.HighchartsAdapter || ($ && {
			...
			$.each(['cur', '_default', 'width', 'height', 'opacity'], function (i, fn) {
				...
				if (base) { // step.width and step.height don't exist in jQuery < 1.7
					...
					obj[fn] = function (fx) {
						...
						// Fx.prototype.cur does not user fx argument
						fx = i ? fx : this;
						...
						return elem.attr ? // is SVG element wrapper
							...
							base.apply(this, arguments); // user jQuery's built-in method
					};
				}
			});
		...
		animate: function (el, params, options) {
			...
			if (params.opacity !== UNDEFINED && el.attr) {
				params.opacity += 'px'; // force jQuery to user same logic as width and height (#2161)
			}
	...
	*/
	...
	on: function (eventType, handler) {
		...
		if (hasTouch && eventType === 'click') {
			...
			// simplest possible event model for internal user
			...
			if (element.namespaceURI === SVG_NS || renderer.forExport) {
				try { // Fails in Firefox if the container has display: none.
					...
					bBox = element.getBBox ?
						// SVG: user extend because IE9 is not allowed to change width and height in case
						// of rotation (below)
...
SVGRenderer.prototype = {
	...
	/**
	 * Dummy function for user in canvas renderer
	 */
	draw: function () {},
	...
	symbol: function (symbol, x, y, width, height, options) {
		...
		if (path) {
			...
			obj = this.path(path);
			// expando properties for user in animate and attr
			extend(obj, {
				symbolName: symbol,
				x: x,
				y: y,
...
var VMLRendererExtension = { // inherit SVGRenderer
	...
	init: function (container, width, height) {
		...
		// The only way to make IE6 and IE7 print is to user a global namespace. However,
	...
	color: function (color, elem, prop, wrapper) {
		...
		if (fillType) {
			...
			each(stops, function (stop, i) {
				...
				// Only start and end opacities are allowed, so we user the first and the last
				if (!i) {
...
Tick.prototype = {
	...
	addLabel: function () {
		...
		// Set the datetime label format. If a higher rank is set for this position, user that. If not,
		// user the general format.
...
function StackItem(axis, options, isNegative, x, stackOption, stacking) {
	...
	// First test the user supplied value, then user the dynamic.
	this.alignOptions = {
...
Axis.prototype = {
	...
	getLogTickPositions: function (interval, min, max, minor) {
		var axis = this,
			options = axis.options,
			axisLength = axis.len,
			// Since we user this method for both major and minor ticks,
			// user a local variable and return the result
			positions = []; 
	...
	setTickPositions: function (secondPass) {
		...
		if (axis.range) {
			...
			if (secondPass) {
				axis.range = null;  // don't user it when running setExtremes
			}
		}
		...
		if (axis.min === axis.max || axis.min === undefined || axis.max === undefined) {
			...
			axis.tickInterval = pick(
				tickIntervalOption,
				categories ? // for categoried axis, 1 is default, for linear axis user tickPix
					1 :
	...
	adjustTickAmount: function () {
		...
		if (maxTicks && maxTicks[key] && !axis.isDatetimeAxis && !axis.categories && !axis.isLinked && axis.options.alignTicks !== false) { // only apply to linear scale
			...
			// set the axis-level tickAmount to user below
	...
	setAxisSize: function () {
		...
		// Expose basic values to user in Series object and navigator
...
Tooltip.prototype = {
	...
	getAnchor: function (points, mouseEvent) {
		...
		// When shared, user the average position
		if (!ret) {
	...
	refresh: function (point, mouseEvent) {
		...
		// get the reference point coordinates (pie charts user tooltipPos)
...
Pointer.prototype = {
	...
	runPointActions: function (e) {
		...
		if (hoverSeries && hoverSeries.tracker) { // only user for line-type series with common tracker
		...
		if (e.type === 'touchstart') {
			...
			each(chart.axes, function (axis) {
				if (axis.zoomEnabled) {
					...
					// Store the bounds for user in the touchmove handler
...
Legend.prototype = {
	...
	render: function () {
		...
		each(chart.series, function (serie) {
			...
			// user points or series for the legend item depending on legendType
...
Chart.prototype = {
	...
	/**
	 * Lay out the chart titles and cache the full offset height for user in getMargins
	 */
...
Series.prototype = {
		...
		if (series.isCartesian) {
			...
			each(['xAxis', 'yAxis'], function (AXIS) { // repeat for xAxis and yAxis
				...
				each(chart[AXIS], function (axis) { // loop through the chart's axis objects
					...
					// axis, or if undefined, user the first axis
	...
	animate: function (init) {
		...
		if (init) { 
			...
			// If a clipping rectangle with the same properties is currently present in the chart, user that.
		...
		while (i--) {
			...
			if (hasPointSpecificOptions) {
				...
				pointAttr[NORMAL_STATE] = series.convertAttribs(extend({
					...
					lineColor: defaultLineColor === null ? point.color : UNDEFINED // Bubbles take point color, line markers user white
	...
	update: function (newOptions, redraw) {
		var chart = this.chart,
			// must user user options when changing type because this.options is merged
	...
	getGraphPath: function () {
		...
		// Record it for user in drawGraph and drawTracker, and return graphPath
	...
	setState: function (state) {
		...
		if (series.state !== state) {
			...
			if (graph && !graph.dashstyle) { // hover is turned off for dashed lines in VML
				...
				// user attr because animate will cause any other animation on the graph to stop
				graph.attr(attribs);
				if (graphNeg) {
	...
	drawTracker: function () {
		...
		// Extend end points. A better way would be to user round linecaps,
...
var ColumnSeries = extendClass(Series, {
	...
	negStacks: true, // user separate negative stacks, unlike area stacks where a negative
				...
				if (minPointLength) {
					...
					barY =
						mathRound(mathAbs(barY - translatedThreshold) > minPointLength ? // stacked
							...
							translatedThreshold - (yAxis.translate(point.y, 0, 1, 0, 1) <= translatedThreshold ? minPointLength : 0)); // user exact yAxis.translation (#1485)
				}
			}
...
var PieSeries = {
	...
	drawPoints: function () {
		...
		each(series.points, function (point) {
			...
			// if the point is sliced, user special translation, else user plot area traslation
	...
	drawDataLabels: function () {
		...
		while (i--) {
			...
			for (j = 0; j < length; j++) {
				...
				// get the x - user the natural x position for first and last slot, to prevent the top
